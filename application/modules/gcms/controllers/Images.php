...
class Images extends AdminController {
	...
	public function index() {
		...
		$config ['base_url'] = ($this->input->get ( "keyword" )) ? base_url () . "gcms/images/index/?keyword=" . $this->input->get ( "keyword" ) : base_url () . "gcms/news/index/";
		// $config ['base_url'] = base_url () . "gcms/user/index/";
		$config ['total_rows'] = ($this->input->get ( "keyword" )) ? $this->Mimages->countSearchImages ( $this->input->get ( "keyword" ), $lang ) : $this->Mimages->countAll ( $lang );
		...
		if ($this->input->get ( "keyword" )) {
			...
			$this->_data ['countSearch'] = $this->Mimages->countSearchImages ( $this->_data ['keyword'], $lang );
			$this->_data ['data'] = $this->Mimages->SearchImagesByKeyword ( $this->_data ['keyword'], $config ['per_page'], $start, $lang );
		} elseif ($locds == "desc" || $locds == "asc") {
			$this->_data ['data'] = $this->Mimages->listAllImages ( $config ['per_page'], $start, $lang, $locds );
		} elseif ($locds == "1" || $locds == "2") {
			$this->_data ['data'] = $this->Mimages->listAllImages ( $config ['per_page'], $start, $lang, "", $locds );
		} else {
			$this->_data ['data'] = $this->Mimages->listAllImages ( $config ['per_page'], $start, $lang );
		}
	...
	public function add() {
		$this->_data ['error'] = "";
		$this->_data ['title'] = "Thêm mới hình ảnh";
...
// 					"type" => $this->input->post ( "type" ),
			...
			// Insert dữ liệu
			$this->Mimages->insertImages ( $data_insert );
			// Flash mess thông báo insert thành công
			$this->session->set_flashdata ( "flash_mess", "Hoàn tất thủ tục thêm hình ảnh." );
	...
	public function edit($id = "") {
		$this->_data ['error'] = "";
		$this->_data ['title'] = "Cập nhật hình ảnh";
		$this->_data ['loadPage'] = "images/edit_view";
		$this->_data ['data'] = $this->Mimages->getImagesById( $id );
		// Validation Form khi nhập sai
		$this->form_validation->set_message ( 'required', '{field} không được để trống.' );
		$this->form_validation->set_message ( 'min_length', '{field} phải nhiều hơn 5 ký tự.' );
		$this->form_validation->set_message ( 'max_length', '{field} phải nhỏ hơn 14 ký tự.' );
...
// 					"type" => $this->input->post ( "type" ),
			...
			// Insert dữ liệu
			$this->Mimages->updateImages ( $id, $data_update );
			// Flash mess thông báo insert thành công
			$this->session->set_flashdata ( "flash_mess", "Hoàn tất thủ sửa hình ảnh." );
			redirect ( base_url () . "gcms/images/index" );
		}
		// echo $this->db->last_query();
		$this->load->view ( $this->_data ['path'], $this->_data );
	...
	public function deleteCB() {
		if ($this->input->post ( "checkAll" )) {
			foreach ( $this->input->post ( "checkAll" ) as $del_id ) {
				$del_id = ( int ) $del_id;
				$this->Mimages->deleteImages ( $del_id );
			}
			$this->session->set_flashdata ( "flash_mess", "Hoàn tất thủ tục xóa hình ảnh." );
			redirect ( base_url () . "gcms/images/index" );
		} else {
			$this->session->set_flashdata ( "flash_error", "Bạn chưa chọn hình ảnh cần xóa." );
			redirect ( base_url () . "gcms/images/index" );
		}
	}
}